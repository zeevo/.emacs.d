...
" nil nil)) (("selection" "An existing jQuery object to add to the set of matched elements.
...
" nil nil))) ("desc" (text . "Create a new jQuery object with elements added to the set of matched
elements.
...
elementʼs appearance in the document. If the collection consists of
...
well-defined order and without sorting overhead, use the
$(array_of_DOM_elements) signature.
...
example, .addBack() adds the previous set of elements on the stack — in
this case $(\"div.after-addback\") — to the current set, selecting both
the div and its enclosed paragraphs.
") (text . "") (js . "
...
" nil nil)) (("function" "A function returning one or more space-separated class names to be
...
" nil nil))) ("desc" (text . "Adds the specified class(es) to each element in the set of matched
elements.

")) ("longdesc" (text . "") (text . "Itʼs important to note that this method does not replace a class. It
simply adds the class, appending it to any which may already be
assigned to the elements.
") (text . "") (text . "The .addClass() method manipulates the className property of the
selected elements, not the class attribute. Once the property is
changed, itʼs the browser that updates the attribute accordingly. An
implication of this behavior is that this method only works for
documents with HTML DOM semantics (e.g., not pure XML documents).
") (text . "") (text . "More than one class may be added at a time, separated by a space, to
the set of matched elements, like so:

") (text . "") (js . "
$( \"p\" ).addClass( \"myClass yourClass\" );
    ") (text . "") (text . "This method is often used with .removeClass() to switch elementsʼ
...
" nil nil)) (("function" "A function that returns an HTML string, DOM element(s), or jQuery
object to insert after each element in the set of matched elements.
Receives the index position of the element in the set and the old HTML
value of the element as arguments. Within the function, this refers to
...
content and target. With .after(), the content to be inserted comes
from the methodʼs argument: $(target).after(contentToBeInserted). With
.insertAfter(), on the other hand, the content precedes the method and
is inserted after the target, which in turn is passed as the
.insertAfter() methodʼs argument:
...
(puthash "ajaxComplete" (quote (("name" . "ajaxComplete") ("signatures" "ajaxComplete" (("handler" "The function to be invoked.
...
(puthash "ajaxError" (quote (("name" . "ajaxError") ("signatures" "ajaxError" (("handler" "The function to be invoked.
...
(puthash "ajaxSend" (quote (("name" . "ajaxSend") ("signatures" "ajaxSend" (("handler" "The function to be invoked.
...
(puthash "ajaxSuccess" (quote (("name" . "ajaxSuccess") ("signatures" "ajaxSuccess" (("handler" "The function to be invoked.
...
traversal stack to be added to the current set. In the first example,
the top stack contains the set resulting from .find(\"p\"). In the second
example, .andSelf() adds the previous set of elements on the stack — in
this case $( \"div.after-andself\" ) — to the current set, selecting both
the div and its enclosed paragraphs.
") (text . "") (js . "
$( \"div.left, div.right\" ).find( \"div, div > p\" ).addClass( \"border\" );
...
" "true" nil) ("complete" "A function to call once the animation is complete, called once per
matched element.
...
fontSize or the CSS equivalent ʼfont-sizeʼ rather than simply ʼfontʼ.
...
element. In order to use jQueryʼs built-in toggle state tracking, the
ʼtoggleʼ keyword must be consistently given as the value of the
...
The strings ʼfastʼ and ʼslowʼ can be supplied to indicate durations of
200 and 600 milliseconds, respectively.
") (text . "") (text . " Callback Functions


") (text . "") (text . "If supplied, the start, step, progress, complete, done, fail, and
always callbacks are called on a per-element basis; this is set to the
DOM element being animated. If no elements are in the set, no callbacks
are called. If multiple elements are animated, the callback is executed
once per matched element, not once for the animation as a whole. Use
the .promise() method to obtain a promise to which you can attach
callbacks that fire once for an animated set of any size, including
zero elements.
...
inserted element will be created for each target except the last, and
...
the attribute is present at all—even if, for example, the attribute has
no value or is set to empty string value or even \"false\". This is true
of all boolean attributes.
...
" nil nil) ("function" "A function returning the value to set. this is the current element.
...
") (text . "") (text . "Note: Attempting to change the type attribute on an input or button
...
major difference is in the syntax—specifically, in the placement of the
content and target. With .before(), the content to be inserted comes
from the methodʼs argument: $(target).before(contentToBeInserted). With
.insertBefore(), on the other hand, the content precedes the method and
is inserted before the target, which in turn is passed as the
.insertBefore() methodʼs argument:
...
" "true" nil) ("handler" "A function to execute each time the event is triggered.
...
" "true" nil)) (("events" "An object containing one or more DOM event types and functions to
execute for them.
...
<span></span>
") (text . "")) ((text . "") (text . "To display each paragraphʼs text in an alert box whenever it is
clicked:
...
(puthash "blur" (quote (("name" . "blur") ("signatures" "blur" (("handler" "A function to execute each time the event is triggered.
...
" "true" nil))) ("desc" (text . "Determine whether or not the list has any callbacks attached. If a
...
(puthash "change" (quote (("name" . "change") ("signatures" "change" (("handler" "A function to execute each time the event is triggered.
...
") (text . "") (text . "Note: Changing the value of an input element using JavaScript, using
.val() for example, wonʼt fire the event.
...
(puthash "click" (quote (("name" . "click") ("signatures" "click" (("handler" "A function to execute each time the event is triggered.
...
") (text . "") (text . "Note: For performance reasons, the dynamic state of certain form
elements (e.g., user data typed into textarea and user selections made
...
instead.
" "true" nil)) (("selection" "A jQuery object to match elements against.
...
similar, except that the former includes text nodes and comment nodes
as well as HTML elements in the resulting jQuery object. Please note
that most jQuery operations donʼt support text nodes and comment nodes.
The few that do will have an explicit note on their API documentation
page.
...
") (text . "")) ((text . "") (text . "Change the background color of links inside of an iframe.
...
(push "contextmenu" jquery-doc-methods)

(puthash "contextmenu" (quote (("name" . "contextmenu") ("signatures" "contextmenu" (("handler" "A function to execute each time the event is triggered.

" nil nil)) (("eventData" "An object containing data that will be passed to the event handler.

" "true" nil) ("handler" "A function to execute each time the event is triggered.

" nil nil)) nil) ("desc" (text . "Bind an event handler to the \"contextmenu\" JavaScript event, or trigger
that event on an element.

")) ("longdesc" (text . "") (text . "This method is a shortcut for .on( \"contextmenu\", handler ) in the
first two variations, and .trigger( \"contextmenu\" ) in the third. The
contextmenu event is sent to an element when the right button of the
mouse is clicked on it, but before the context menu is displayed. In
case the context menu key is pressed, the event is triggered on the
html element. Any HTML element can receive this event. For example,
consider the HTML:
") (text . "") (js . "
<div id=\"target\">
  Right-click here
</div>
    ") (text . "") (text . "The event handler can be bound to the <div> as follows:
...
$( \"#target\" ).contextmenu(function() {
  alert( \"Handler for .contextmenu() called.\" );
    ...
    ") (text . "") (text . "Now right-clicking on this element displays the alert:


") (text . "") (text . "Handler for .contextmenu() called.


") (text . "") (text . "To trigger the event manually, call .contextmenu() without an argument:


") (text . "") (js . "
$( \"#target\" ).contextmenu();
    ") (text . "")) ("examples" ((text . "") (text . "To show a \"Hello World!\" alert box when the contextmenu event is
triggered on a paragraph on the page:

") (text . "") (js . "
$( \"p\" ).contextmenu(function() {
  alert( \"Hello World!\" );
});
") (text . "")) ((text . "") (text . "Right click to toggle background color.
...
" nil nil))) ("desc" (text . "Get the computed style properties for the first element in the set of
matched elements.

")) ("longdesc" (text . "") (text . "The .css() method is a convenient way to get a computed style property
from the first matched element, especially in light of the different
ways browsers access most of those properties (the getComputedStyle()
...
browsers use for certain properties. For example, Internet Explorerʼs
...
\"backgroundColor\" ). This means mixed case has a special meaning, .css(
\"WiDtH\" ) wonʼt do the same as .css( \"width\" ), for example.
") (text . "") (text . "Note that the computed style of an element may not be the same as the
value specified for that element in a style sheet. For example,
computed styles of dimensions are almost always pixels, but they can be
specified as em, ex, px or % in a style sheet. Different browsers may
return CSS color values that are logically but not textually equal,
e.g., #FFF, #ffffff, and rgb(255,255,255).
...
and so on.
") (text . "") (text . "An element should be connected to the DOM when calling .css() on it. If
it isnʼt, jQuery may throw an error.
...
") (text . "") (text . "When a number is passed as the value, jQuery will convert it to a
...
manipulation of the style property. As a consequence, the elementʼs
style for that property will be restored to whatever value was applied.
So, this method can be used to cancel any style modification you have
previously performed. It does not, however, remove a style that has
been applied with a CSS rule in a stylesheet or <style> element.
Warning: one notable exception is that, for IE 8 and below, removing a
shorthand property such as border or background will remove that style
entirely from the element, regardless of what is set in a stylesheet or
<style> element.
") (text . "") (text . "As of jQuery 1.8, the .css() setter will automatically take care of
prefixing the property name. For example, take .css( \"user-select\",
\"none\" ) in Chrome/Safari will set it as -webkit-user-select, Firefox
will use -moz-user-select, and IE10 will use -ms-user-select.
...
" nil nil) ("value" "The new data value; this can be any Javascript type except undefined.
...
$( \"div\" ).data( \"options\" ).name === \"John\";
      ") (text . "") (text . "The second statement of the code above correctly refers to the
...
converted to a number if doing so doesnʼt change the valueʼs
...
") (text . "") (text . "When the data attribute is an object (starts with ʼ{ʼ) or array (starts
with ʼ[ʼ) then jQuery.parseJSON is used to parse the string; it must
follow valid JSON syntax including quoted property names. If the value
isnʼt parseable as a JavaScript value, it is left as a string.
") (text . "") (text . "To retrieve the valueʼs attribute as a string without any attempt to
...
(puthash "dblclick" (quote (("name" . "dblclick") ("signatures" "dblclick" (("handler" "A function to execute each time the event is triggered.
...
") (text . "") (text . "To trigger the event manually, call .dblclick() without an argument:
...
") (text . "")) ("examples" ((text . "") (text . "To bind a \"Hello World!\" alert box to the dblclick event on every
...
for Deferred object.
") (text . "") (text . "Note: The deferred.always() method receives the arguments that were
used to .resolve() or .reject() the Deferred object, which are often
very different. For this reason, itʼs best to use it only for actions
that do not require inspecting the arguments. In all other cases, use
explicit .done() or .fail() handlers since the arguments will have
well-known orders.
...
can prevent other code from changing the Deferredʼs state or reporting
...
" nil nil) ("args" "An optional array of arguments that are passed to the
progressCallbacks.
...
progressFilter function to filter any calls to the original deferredʼs
notify or notifyWith methods. These filter functions can return a new
value to be passed along to the piped promiseʼs done() or fail()
...
to the piped promiseʼs callbacks. If the filter function used is null,
...
" nil nil) ("progressCallbacks" "Optional additional function, or array of functions, to be called when
the Deferred generates progress notifications.

" "true" nil))) ("desc" (text . "Add handlers to be called when the Deferred object generates progress
notifications.

")) ("longdesc" (text . "") (text . "The deferred.progress() method accepts one or more arguments, all of
which can be either a single function or an array of functions. When
the Deferred generates progress notifications by calling notify or
...
fail, always, pipe, progress, state and promise), but not ones that
change the state ( resolve, reject, notify, resolveWith, rejectWith,
and notifyWith).
...
can prevent other code from changing the Deferredʼs state by returning
a restricted Promise object through deferred.promise() .
") (text . "") (text . "When the Deferred is resolved, any doneCallbacks added by
...
original deferredʼs notify or notifyWith methods. These filter
functions can return a new value to be passed along to the promiseʼs
...
status and values to the promiseʼs callbacks. If the filter function
...
Because it is limited—it doesnʼt, for example, offer a way to cancel
the delay— .delay() is not a replacement for JavaScriptʼs native
...
(puthash "each" (quote (("name" . "each") ("signatures" "each" (("function" "A function to execute for each matched element.
...
")) ("longdesc" (text . "") (text . "Most of jQueryʼs DOM traversal methods operate on a jQuery object
...
" nil nil)) (("indexFromEnd" "An integer indicating the position of the element, counting backwards
...
(puthash "error" (quote (("name" . "error") ("signatures" "error" (("handler" "A function to execute when the event is triggered.
...
") (text . "") (text . "As of jQuery 1.8, the .error() method is deprecated. Use .on( \"error\",
handler ) to attach event handlers to the error event instead.
...
") (text . "")) ("examples" ((text . "") (text . "To replace all the missing images with another, you can update the src
attribute inside the callback passed to .error(). Be sure that the
replacement image exists; otherwise the error event will be triggered
indefinitely.
") (text . "") (js . "
...
" nil nil)) (("function" "A function used as a test for each element in the set. this is the
current DOM element.

" nil nil)) (("elements" "One or more DOM elements to match the current set of elements against.

" nil nil)) (("selection" "An existing jQuery object to match the current set of elements against.
...
" nil nil)) (("element" "An element or a jQuery object to match elements against.
...
the match.
") (text . "") (text . "Unlike most of the tree traversal methods, the selector expression is
required in a call to .find(). If we need to retrieve all of the
descendant elements, we can pass in the universal selector ʼ*ʼ to
accomplish this.
...
(puthash "focus" (quote (("name" . "focus") ("signatures" "focus" (("handler" "A function to execute each time the event is triggered.
...
To run an elementʼs focus event handlers without setting focus to the
...
(puthash "focusin" (quote (("name" . "focusin") ("signatures" "focusin" (("handler" "A function to execute each time the event is triggered.

" nil nil)) (("eventData" "An object containing data that will be passed to the event handler.

" "true" nil) ("handler" "A function to execute each time the event is triggered.

" nil nil)) nil) ("desc" (text . "Bind an event handler to the \"focusin\" event.

")) ("longdesc" (text . "") (text . "This method is a shortcut for .on( \"focusin\", handler ) in the first
two variations, and .trigger( \"focusin\" ) in the third.
...
(puthash "focusout" (quote (("name" . "focusout") ("signatures" "focusout" (("handler" "A function to execute each time the event is triggered.

" nil nil)) (("eventData" "An object containing data that will be passed to the event handler.

" "true" nil) ("handler" "A function to execute each time the event is triggered.

" nil nil)) nil) ("desc" (text . "Bind an event handler to the \"focusout\" JavaScript event.
...
" nil nil))) ("desc" (text . "Retrieve one of the elements matched by the jQuery object.

")) ("longdesc" (text . "") (text . "The .get() method grants access to the DOM nodes underlying each jQuery
object. If the value of index is out of bounds — less than the negative
number of elements or equal to or greater than the number of elements —
it returns undefined. Consider a simple unordered list:
") (text . "") (html . "
<ul>
  <li id=\"foo\">foo</li>
  <li id=\"bar\">bar</li>
...
(puthash "get" (quote (("name" . "get") ("signatures" "get" nil) ("desc" (text . "Retrieve the elements matched by the jQuery object.
...
") (text . "") (js . "
$( \"#mydiv\" ).hasClass( \"quux\" )
    ") (text . "")) ("examples" ((text . "") (text . "Looks for the paragraph that contains ʼselectedʼ as a class.
...
.height() method is recommended when an elementʼs height needs to be
used in a mathematical calculation.
") (text . "") (text . " [0042_04_01.png] figure 1
...
" nil nil)) (("function" "A function returning the height to set. Receives the index position of
...
important to note that the callback is executed once per matched
element, not once for the animation as a whole.
") (text . "") (text . "Note: This method may cause performance issues, especially when used on
many elements. If youʼre encountering such issues, use performance
testing tools to determine whether this method is causing them.
Moreover, this method can cause problems with responsive layouts if the
display value differs at different viewport sizes.
") (text . "") (text . "We can animate any element, such as a simple image:
...
(puthash "hover" (quote (("name" . "hover") ("signatures" "hover" (("handlerIn" "A function to execute when the mouse pointer enters the element.

" nil nil) ("handlerOut" "A function to execute when the mouse pointer leaves the element.
...
(puthash "hover" (quote (("name" . "hover") ("signatures" "hover" (("handlerInOut" "A function to execute when the mouse pointer enters or leaves the
element.
...
$( \"div.demo-container\" ).html();
      ") (text . "") (text . "In order for the following <div>ʼs content to be retrieved, it would
...
" nil nil)) (("function" "A function returning the HTML content to set. Receives the index
...
") (text . "") (text . "To set the content of a <script> element, which does not contain HTML,
use the .text() method and not .html().
...
") (text . "") (text . " [0042_04_02.png] figure 1
...
(puthash "innerHeight" (quote (("name" . "innerHeight") ("signatures" "innerHeight" (("value" "A number representing the number of pixels, or a number along with an
optional unit of measure appended (as a string).

" nil nil)) (("function" "A function returning the inner height (including padding but not
border) to set. Receives the index position of the element in the set
and the old inner height as arguments. Within the function, this refers
to the current element in the set.
" nil nil))) ("desc" (text . "Set the CSS inner height of each element in the set of matched
elements.

")) ("longdesc" (text . "") (text . "When calling .innerHeight(\"value\"), the value can be either a string
(number and unit) or a number. If only a number is provided for the
value, jQuery assumes a pixel unit. If a string is provided, however,
any valid CSS measurement may be used for the height (such as 100px,
50%, or auto). Note that in modern browsers, the CSS height property
does not include padding, border, or margin, unless the box-sizing CSS
property is used.
") (text . "") (text . "If no explicit unit is specified (like \"em\" or \"%\") then \"px\" is
assumed.

") (text . "")) ("examples" ((text . "") (text . "Change the inner height of each div the first time it is clicked (and
change its color).
...
(puthash "innerWidth" (quote (("name" . "innerWidth") ("signatures" "innerWidth" nil) ("desc" (text . "Get the current computed inner width for the first element in the set
...
") (text . "") (text . " [0042_04_05.png] figure 1
...
(puthash "innerWidth" (quote (("name" . "innerWidth") ("signatures" "innerWidth" (("value" "A number representing the number of pixels, or a number along with an
optional unit of measure appended (as a string).

" nil nil)) (("function" "A function returning the inner width (including padding but not border)
to set. Receives the index position of the element in the set and the
old inner width as arguments. Within the function, this refers to the
current element in the set.
" nil nil))) ("desc" (text . "Set the CSS inner width of each element in the set of matched elements.


")) ("longdesc" (text . "") (text . "When calling .innerWidth(\"value\"), the value can be either a string
(number and unit) or a number. If only a number is provided for the
value, jQuery assumes a pixel unit. If a string is provided, however,
any valid CSS measurement may be used for the width (such as 100px,
50%, or auto). Note that in modern browsers, the CSS width property
does not include padding, border, or margin, unless the box-sizing CSS
property is used.
") (text . "") (text . "If no explicit unit is specified (like \"em\" or \"%\") then \"px\" is
assumed.

") (text . "")) ("examples" ((text . "") (text . "Change the inner width of each div the first time it is clicked (and
change its color).
...
" nil nil)) (("function" "A function used as a test for every element in the set. It accepts two
arguments, index, which is the elementʼs index in the jQuery
collection, and element, which is the DOM element. Within the function,
this refers to the current DOM element.
" nil nil)) (("selection" "An existing jQuery object to match the current set of elements against.


" nil nil)) (("elements" "One or more elements to match the current set of elements against.
...
ability to remove a particular callback from the callback list. Hereʼs
...
means itʼs possible to combine flags to create callback lists that,
...
") (text . "") (text . "$.Callbacks( ʼunique memoryʼ ):
...
itʼs possible to implement a Pub/Sub system using only callback lists.
...
$.Deferreds, itʼs possible to ensure publishers only publish
...
" "true" nil))) ("desc" (text . "A factory function that returns a chainable utility object with methods
...
")) ("longdesc" (text . "") (text . "The jQuery.Deferred() factory creates a new deferred object.


") (text . "") (text . "The jQuery.Deferred method can be passed an optional function, which is
called just before the method returns and is passed the new deferred
object as both the this object and as the first argument to the
function. The called function can attach callbacks using
...
Promise-compatible object, such as jQuery.ajax() or jQuery.when() , you
will only want to use the deferred.then() , deferred.done() , and
deferred.fail() methods to add callbacks to the Deferredʼs queues. The
...
") (text . "") (text . "  * jqXHR.done(function( data, textStatus, jqXHR ) {});
  ...
  * jqXHR.always(function( data|jqXHR, textStatus, jqXHR|errorThrown )
    ...
    In response to a successful request, the functionʼs arguments are
...
") (text . "") (text . "Different types of response to $.ajax() call are subjected to different
...
response, but can be set explicitly using the dataType option. If the
dataType option is provided, the Content-Type header of the response
will be disregarded.
") (text . "") (text . "The available data types are text, html, xml, json, jsonp, and script.


") (text . "") (text . "If text or html is specified, no pre-processing occurs. The data is
simply passed on to the success handler, and made available through the
responseText property of the jqXHR object.
") (text . "") (text . "If xml is specified, the response is parsed using jQuery.parseXML
before being passed, as an XMLDocument , to the success handler. The
XML document is made available through the responseXML property of the
jqXHR object.
") (text . "") (text . "If json is specified, the response is parsed using jQuery.parseJSON
...
jqXHR object.
") (text . "") (text . "If script is specified, $.ajax() will execute the JavaScript that is
received from the server before passing it on to the success handler as
a string.
") (text . "") (text . "If jsonp is specified, $.ajax() will automatically append a query
string parameter of (by default) callback=? to the URL. The jsonp and
jsonpCallback properties of the settings passed to $.ajax() can be used
to specify, respectively, the name of the query string parameter and
...
") (text . "") (text . "For more information on JSONP, see the original post detailing its use.
...
key1=value1&key2=value2, or an object of the form {key1: ʼvalue1ʼ,
key2: ʼvalue2ʼ}. If the latter form is used, the data is converted into
...
these methods below for more details.
...
you must add a correspondence between the response Content-Type and the
...
") (text . "")) ("examples" ((text . "") (text . "Save some data to the server and notify the user once itʼs complete.
...
notify the user once itʼs complete. If the request fails, alert the
user.
") (text . "") (js . "
...
" "true" nil) ("handler" "A handler to set default values for future Ajax requests.
...
" nil nil) ("handler" "A handler to return the new transport object to use with the data type
...
") (text . "") (text . "  * status is the HTTP status code of the response, like 200 for a
  ...
  * responses (Optional) is An object containing dataType/value that
    ...
    responses to { xml: XMLData, text: textData } for a response that
    is an XML document)
...
") (text . "") (text . "Just like prefilters, a transportʼs factory function can be attached to
a specific dataType:
...
returned.
") (text . "") (text . "Calling jQuery.data( element ) retrieves all of the elementʼs
...
(puthash "$.each" (quote (("name" . "$.each") ("signatures" "$.each" (("array" "The array to iterate over.

" nil nil) ("callback" "The function that will be executed on every object.

" nil nil)) (("object" "The object to iterate over.
...
was iterated.
") (text . "") (text . "Note: The $.each() function internally retrieves and uses the length
property of the passed collection. So, if the collection has a property
called length — e.g. {bar: ʼfooʼ, length: 10} — the function might not
work as expected.
") (text . "") (js . "
...
the objectʼs prototype will be copied over. Properties that are an
...
method instead, or use a library like lodash.
...
(puthash "$.get" (quote (("name" . "$.get") ("signatures" "$.get" (("settings" "A set of key/value pairs that configure the Ajax request. All
...
list of all settings. The type option will automatically be set to GET.
" "false" nil)) (("url" "A string containing the URL to which the request is sent.

" nil nil) ("data" "A plain object or string that is sent to the server with the request.

" "true" nil) ("success" "A callback function that is executed if the request succeeds. Required
if dataType is provided, but you can use null or jQuery.noop as a
placeholder.
...
") (text . "") (text . "The Promise interface also allows jQueryʼs Ajax methods, including
...
" "true" nil) ("success" "A callback function that is executed if the request succeeds.
...
syntax rules that are stricter than those of JavaScriptʼs object
...
" nil nil) ("success" "A callback function that is executed if the request succeeds.
...
(puthash "$.grep" (quote (("name" . "$.grep") ("signatures" "$.grep" (("array" "The array-like object to search through.

" nil nil) ("function" "The function to process each item against. The first argument to the
...
") (text . "") (text . "Note that jQueryʼs event system uses the jQuery data API to store event
...
")) ("longdesc" (text . "") (text . "The $.inArray() method is similar to JavaScriptʼs native .indexOf()
...
but 0 !== false), to check for the presence of value within array, you
need to check if itʼs not equal to (or greater than) -1.
") (text . "") (text . "The comparison between values is strict. The following will return -1
(not found) because a number is being searched in an array of strings:

") (text . "") (js . "$.inArray( 5 + 5, [ \"8\", \"9\", \"10\", 10 + \"\" ] );") (text . "")) ("examples" ((text . "") (text . "Report the index of some elements in the array.
...
$.isPlainObject()
") (text . "")) ("examples" ((text . "") (text . "Check an object to see if itʼs empty.
...
itʼs important to be aware of any of the gotchas involved in using
$.isPlainObject() against older browsers. A couple basic examples that
do function correctly cross-browser can be found below.
") (text . "")) ("examples" ((text . "") (text . "Check an object to see if itʼs a plain object.
...
")) ("longdesc" (text . "") (text . "This is used in a number of places in jQuery to determine if weʼre
...
" nil nil) ("callback" "The function to process each item against. The first argument to the
...
" nil nil)) (("object" "The Object to translate.

" nil nil) ("callback" "The function to process each item against. The first argument to the
function is the value; the second argument is the key of the object
property. The function can return any value to add to the array. A
returned array will be flattened into the resulting array. Within the
...
")) ("longdesc" (text . "") (text . "If you wish to process a jQuery object — for example, $(ʼdivʼ).map(
...
(puthash "$.merge" (quote (("name" . "$.merge") ("signatures" "$.merge" (("first" "The first array-like object to merge, the elements of second added.

" nil nil) ("second" "The second array-like object to merge into the first, unaltered.
...
destructive. It alters the length and numeric index properties of the
first object to include items from the second.
...
the $ as a jQuery alias inside the functionʼs scope. Inside the
function the original $ object is not available. This works well for
most plugins that donʼt rely on any other library.
") (text . "") (js . "
jQuery.noConflict();
(function( $ ) {
  $(function() {
...
p\" ).hide();
") (text . "")) ((text . "") (text . "Load two versions of jQuery (not recommended). Then, restore jQueryʼs
...
(puthash "$.param" (quote (("name" . "$.param") ("signatures" "$.param" (("obj" "An array, a plain object, or a jQuery object to serialize.

" nil nil)) (("obj" "An array, a plain object, or a jQuery object to serialize.

" nil nil) ("traditional" "A Boolean indicating whether to perform a traditional \"shallow\"
serialization.

" nil nil))) ("desc" (text . "Create a serialized representation of an array, a plain object, or a
...
")) ("longdesc" (text . "") (text . "jQuery.parseHTML uses native methods to convert the string to a set of
DOM nodes, which can then be inserted into the document. These methods
do render all trailing or leading text (even if thatʼs just
whitespace). To prevent trailing/leading whitespace from being
converted to text nodes you can pass the HTML string through
jQuery.trim .
...
document such as an iframe, that frameʼs document could be used.
") (text . "") (text . "Security Considerations


") (text . "") (text . "Most jQuery APIs that accept HTML strings will run scripts that are
included in the HTML. jQuery.parseHTML does not run scripts in the
parsed HTML unless keepScripts is explicitly true. However, it is still
possible in most environments to execute scripts indirectly, for
example via the <img onerror> attribute. The caller should be aware of
this and guard against it by cleaning or escaping any untrusted inputs
from sources such as the URL or cookies. For future compatibility,
callers should not depend on the ability to run any script content when
keepScripts is unspecified or false.
...
being thrown. For example, the following are all invalid JSON strings:

") (text . "") (text . "  * \"{test: 1}\" (test does not have double quotes around it).
  * \"{ʼtestʼ: 1}\" (ʼtestʼ is using single quotes instead of double
    quotes).
  * \"ʼtestʼ\" (ʼtestʼ is using single quotes instead of double quotes).
  * \".1\" (a number must start with a digit; \"0.1\" would be valid).
  * \"undefined\" ( undefined cannot be represented in a JSON string;
    null, however, can be).
  * \"NaN\" ( NaN cannot be represented in a JSON string; direct
    representation of Infinity is also not permitted).
") (text . "") (text . "The JSON standard does not permit \"control characters\" such as a tab or
newline. An example like $.parseJSON( ʼ{ \"testing\":\"1\\t2\\n3\" }ʼ ) will
throw an error in most implementations because the JavaScript parser
converts the stringʼs tab and newline escapes into literal tab and
...
" "true" nil)) (("settings" "A set of key/value pairs that configure the Ajax request. All
properties except for url are optional. A default can be set for any
option with $.ajaxSetup(). See jQuery.ajax( settings ) for a complete
list of all settings. Type will automatically be set to POST.
" "false" nil))) ("desc" (text . "Load data from the server using a HTTP POST request.
...
original.
") (text . "") (text . "Be aware, however, that jQueryʼs event binding subsystem assigns a
...
during unbinding.
") (text . "") (text . "As of jQuery 1.6, any number of additional arguments may be supplied to
...
") (text . "")) ("examples" ((text . "") (text . "Adding a method to a jQuery sub so that it isnʼt exposed externally:
...
jQuery.ajax() is a Promise-compatible object and can be used this way:
") (text . "") (js . "
$.when( $.ajax( \"test.aspx\" ) ).then(function( data, textStatus, jqXHR ) {
  alert( jqXHR.status ); // Alerts 200
});
    ") (text . "") (text . "If a single argument is passed to jQuery.when() and it is not a
Deferred or a Promise, it will be treated as a resolved Deferred and
any doneCallbacks attached will be executed immediately. The
doneCallbacks are passed the original argument. In this case any
failCallbacks you might set are never called since the Deferred is
never rejected. For example:
") (text . "") (js . "
$.when( { testing: 123 } ).done(function( x ) {
  alert( x.testing ); // Alerts \"123\"
});
    ") (text . "") (text . "If you donʼt pass it any arguments at all, jQuery.when() will return a
resolved promise.
...
$.when().then(function( x ) {
  alert( \"I fired immediately\" );
    ...
    ") (text . "") (text . "In the case where multiple Deferred objects are passed to
jQuery.when(), the method returns the Promise from a new \"master\"
Deferred object that tracks the aggregate state of all the Deferreds it
has been passed. The method will resolve its master Deferred as soon as
all the Deferreds resolve, or reject the master Deferred as soon as one
of the Deferreds is rejected. If the master Deferred is resolved, the
doneCallbacks for the master Deferred are executed. The arguments
passed to the doneCallbacks provide the resolved values for each of the
Deferreds, and matches the order the Deferreds were passed to
jQuery.when(). For example:
") (text . "") (js . "
var d1 = $.Deferred();
var d2 = $.Deferred();
...
d1.resolve( \"Fish\" );
d2.resolve( \"Pizza\" );
    ") (text . "") (text . "In the event a Deferred was resolved with no value, the corresponding
doneCallback argument will be undefined. If a Deferred resolved to a
single value, the corresponding argument will hold that value. In the
case where a Deferred resolved to multiple values, the corresponding
argument will be an array of those values. For example:
") (text . "") (js . "
var d1 = $.Deferred();
var d2 = $.Deferred();
var d3 = $.Deferred();
...
that point. The arguments passed to the failCallbacks match the
signature of the failCallback for the Deferred that was rejected. If
you need to perform additional processing for this case, such as
canceling any unfinished Ajax requests, you can keep references to the
underlying jqXHR objects in a closure and inspect/cancel them in the
failCallback.
...
" nil nil)) (("selection" "An existing jQuery object to clone.
...
some other way. A jQuery object is created from the array elements in
the order they appeared in the array; unlike most other multi-element
jQuery operations, the elements are not sorted in DOM order. Elements
will be copied from the array as-is and wonʼt be unwrapped if theyʼre
already jQuery collections.
") (text . "") (text . "Please note that although you can pass text nodes and comment nodes
into a jQuery collection this way, most operations donʼt support them.
The few that do will have an explicit note on their API documentation
page.
") (text . "") (text . "A common use of single-DOM-element construction is to call jQuery
methods on an element that has been passed to a callback function
...
the browserʼs .innerHTML property to parse the passed HTML and insert
...
string passed.
") (text . "") (text . "Filtering isnʼt, however, limited to these tags. For example, Internet
...
$( \"<img>\" );
$( \"<input>\" );
      ") (text . "") (text . "When passing HTML to jQuery(), note that text nodes are not treated as
DOM elements. With the exception of a few methods (such as .content()),
they are generally ignored or removed. E.g:
") (text . "") (js . "
var el = $( \"<br>2<br>3\" ); // returns [<br>, \"2\", <br>]
el = $( \"<br>2<br>3 >\" ); // returns [<br>, \"2\", <br>, \"3 &gt;\"]
      ") (text . "") (text . "This behavior is expected. As of jQuery 1.9.0 (and unless using the
jQuery Migrate plugin), jQuery() requires the HTML string to start with
...
first argument must represent a simple element with no attributes. As
...
there really isnʼt much use for chaining against it.
...
(puthash "keydown" (quote (("name" . "keydown") ("signatures" "keydown" (("handler" "A function to execute each time the event is triggered.
...
(puthash "keypress" (quote (("name" . "keypress") ("signatures" "keypress" (("handler" "A function to execute each time the event is triggered.
...
(http://api.jquery.com/resources/events.js) for the event objectʼs
output.
") (text . "") (js . "
var xTriggered = 0;
...
(puthash "keyup" (quote (("name" . "keyup") ("signatures" "keyup" (("handler" "A function to execute each time the event is triggered.
...
") (text . "") (text . "  * Before jQuery 1.7, to stop further handlers from executing after
  ...
  * As of jQuery 1.4 the .live() method supports custom events as well
    ...
    events that donʼt bubble, including change, submit, focus and blur.
...
(puthash "load" (quote (("name" . "load") ("signatures" "load" (("handler" "A function to execute when the event is triggered.
...
the parent document and the event.target isnʼt set by Firefox, IE9 or
...
" "true" nil) ("complete" "A callback function that is executed when the request completes.
...
$( \"#b\" ).load( \"article.html #target\" );
    ") (text . "")) ("examples" ((text . "") (text . "Load another pageʼs list items into an ordered list.
...
(puthash "map" (quote (("name" . "map") ("signatures" "map" (("callback" "A function object that will be invoked for each element in the current
set.
...
") (text . "") (text . "As the return value is a jQuery object, which contains an array, itʼs
...
(puthash "mousedown" (quote (("name" . "mousedown") ("signatures" "mousedown" (("handler" "A function to execute each time the event is triggered.
...
(puthash "mouseenter" (quote (("name" . "mouseenter") ("signatures" "mouseenter" (("handler" "A function to execute each time the event is triggered.
...
<div id=\"log\"></div>") (text . "") (text . " figure 1
...
(puthash "mouseleave" (quote (("name" . "mouseleave") ("signatures" "mouseleave" (("handler" "A function to execute each time the event is triggered.
...
")) ("longdesc" (text . "") (text . "This method is a shortcut for .on(ʼmouseleaveʼ, handler) in the first
...
(puthash "mousemove" (quote (("name" . "mousemove") ("signatures" "mousemove" (("handler" "A function to execute each time the event is triggered.
...
(puthash "mouseout" (quote (("name" . "mouseout") ("signatures" "mouseout" (("handler" "A function to execute each time the event is triggered.
...
(puthash "mouseover" (quote (("name" . "mouseover") ("signatures" "mouseover" (("handler" "A function to execute each time the event is triggered.
...
(puthash "mouseup" (quote (("name" . "mouseup") ("signatures" "mouseup" (("handler" "A function to execute each time the event is triggered.
...
")) ("longdesc" (text . "") (text . "This method is a shortcut for .on(ʼmouseupʼ, handler) in the first
...
(puthash "not" (quote (("name" . "not") ("signatures" "not" (("selector" "A string containing a selector expression, a DOM element, or an array
of elements to match against the set.

" nil nil)) (("function" "A function used as a test for each element in the set. It accepts two
arguments, index, which is the elementʼs index in the jQuery
collection, and element, which is the DOM element. Within the function,
this refers to the current DOM element.
" nil nil)) (("selection" "An existing jQuery object to match the current set of elements against.
...
" nil nil)) nil) ("desc" (text . "Remove an event handler.
...
(puthash "offset" (quote (("name" . "offset") ("signatures" "offset" (("coordinates" "An object containing the properties top and left, which are numbers
indicating the new top and left coordinates for the elements.

" nil nil)) (("function" "A function to return the coordinates to set. Receives the index of the
...
through the browserʼs standard JavaScript event types, calling the
...
$( \"#dataTable tbody tr\" ).on( \"click\", function() {
  console.log( $( this ).text() );
    ...
    ") (text . "") (text . "An event-delegation approach attaches an event handler to only one
...
$( \"button\" ).on( \"click\", notify );
    ") (text . "") (text . "When the browser triggers an event or other JavaScript calls jQueryʼs
...
normalized subset of data provided by the browser; the browserʼs
...
argument to .trigger() or .triggerHandler() . Data provided this way is
passed to the event handler as further parameters after the Event
object. If an array was passed to the second argument of .trigger() or
.triggerHandler(), each element in the array will be presented to the
event handler as an individual parameter.
...
") (text . "") (text . "jQueryʼs event system requires that a DOM element allow attaching data
...
") (text . "") (text . "The handler list for an element is set when the event is first
delivered. Adding or removing event handlers on the current element
wonʼt take effect until the next time the event is handled. To prevent
any further event handlers from executing on an element within an event
handler, call event.stopImmediatePropagation(). This behavior goes
against the W3C events specification. To better understand this case,
consider the following code:
") (text . "") (js . "var $test = $( \"#test\" );
...
$test.on( \"click\", handler2 );") (text . "") (text . "In the code above, handler2 will be executed anyway the first time even
if itʼs removed using .off() . However, the handler will not be
executed the following times the click event is triggered.
") (text . "")) ("examples" ((text . "") (text . "Display a paragraphʼs text in an alert when it is clicked:
...
") (text . "")) ((text . "") (text . "Pass data to the event handler using the second argument to .trigger()
...
") (text . "")) ((text . "") (text . "Use the the second argument of .trigger() to pass an array of data to
the event handler
...
<p>Click me!</p>
<span></span>
") (text . "")) ((text . "") (text . "Display each paragraphʼs text in an alert box whenever it is clicked:

") (text . "") (js . "
$( \"body\" ).on( \"click\", \"p\", function() {
  alert( $( this ).text() );
});
") (text . "")) ((text . "") (text . "Cancel a linkʼs default action using the .preventDefault() method.
...
")) ("longdesc" (text . "") (text . "The .one() method is identical to .on(), except that the handler is
unbound after its first invocation. For example:
...
") (text . "") (text . " [0042_04_03.png] figure 1
...
(puthash "outerHeight" (quote (("name" . "outerHeight") ("signatures" "outerHeight" (("value" "A number representing the number of pixels, or a number along with an
optional unit of measure appended (as a string).

" nil nil)) (("function(index, height)" "A function returning the outer height to set. Receives the index
position of the element in the set and the old outer height as
arguments. Within the function, this refers to the current element in
the set.
" nil nil))) ("desc" (text . "Set the CSS outer Height of each element in the set of matched
elements.

")) ("longdesc" (text . "") (text . "When calling .outerHeight(value), the value can be either a string
(number and unit) or a number. If only a number is provided for the
value, jQuery assumes a pixel unit. If a string is provided, however,
any valid CSS measurement may be used (such as 100px, 50%, or auto).
") (text . "")) ("examples" ((text . "") (text . "Change the outer height of each div the first time it is clicked (and
change its color).
...
") (text . "") (text . " [0042_04_06.png] figure 1
...
(puthash "outerWidth" (quote (("name" . "outerWidth") ("signatures" "outerWidth" (("value" "A number representing the number of pixels, or a number along with an
optional unit of measure appended (as a string).

" nil nil)) (("function(index, width)" "A function returning the outer width to set. Receives the index
position of the element in the set and the old outer width as
arguments. Within the function, this refers to the current element in
the set.
" nil nil))) ("desc" (text . "Set the CSS outer width of each element in the set of matched elements.


")) ("longdesc" (text . "") (text . "When calling .outerWidth(value), the value can be either a string
(number and unit) or a number. If only a number is provided for the
value, jQuery assumes a pixel unit. If a string is provided, however,
any valid CSS measurement may be used (such as 100px, 50%, or auto).
") (text . "")) ("examples" ((text . "") (text . "Change the outer width of each div the first time it is clicked (and
change its color).
...
")) ("longdesc" (text . "") (text . "Given a jQuery object that represents a set of DOM elements, the
parent() method traverses to the immediate parent of each of these
elements in the DOM tree and constructs a new jQuery object from the
matching elements.
") (text . "") (text . "This method is similar to .parents() , except .parent() only travels a
single level up the DOM tree. Also, $( \"html\" ).parent() method returns
a set containing document whereas $( \"html\" ).parents() returns an
empty set.
") (text . "") (text . "The method optionally accepts a selector expression of the same type
that we can pass to the $() function. If the selector is supplied, the
elements will be filtered by testing whether they match it.
...
<div class=\"container\">
  ...
  <div class=\"inner\">Goodbye</div>
    ...
    ") (text . "") (text . "If there is more than one target element, however, cloned copies of the
inserted element will be created for each target except the last.
...
elementʼs data as well as the element itself, it will prevent any of
the elementʼs unresolved Promises from resolving. If it is necessary to
...
" nil nil) ("function" "A function returning the value to set. Receives the index position of
...
" "true" nil) ("callback" "The new function to add to the queue, with a function to call that will
...
executes.
") (text . "") (text . "As of jQuery 1.4, the function thatʼs called is passed another function
...
jQueryʼs .load() method to attach load event handlers to the window or
...
doesnʼt achieve the desired effect in Internet Explorer 6, 7, or 8. To
...
" "true" nil)) (("function" "A function returning one or more space-separated class names to be
...
") (text . "") (text . "The .removeClass() method manipulates the className property of the
selected elements, not the class attribute. Once the property is
changed, itʼs the browser that updates the attribute accordingly. This
means that when the class attribute is updated and the last class name
is removed, the browser may set the attributeʼs value to an empty
string instead of removing the attribute completely. An implication of
this behavior is that this method only works for documents with HTML
DOM semantics (e.g., not pure XML documents).
...
") (text . "")) ((text . "") (text . "Remove the class ʼblueʼ and ʼunderʼ from the matched elements.
...
arguments, .removeData() removes all values.
") (text . "") (text . "Note that .removeData() will only remove data from jQueryʼs internal
...
the value from the data- attribute. To prevent this, use .removeAttr()
alongside .removeData() to remove the data- attribute as well. Prior to
jQuery 1.4.3, as data() did not use data- attributes, this was not an
issue.
...
")) ("longdesc" (text . "") (text . "The .replaceAll() method is similar to .replaceWith() , but with the
...
(puthash "resize" (quote (("name" . "resize") ("signatures" "resize" (("handler" "A function to execute each time the event is triggered.
...
")) ("longdesc" (text . "") (text . "This method is a shortcut for .on(ʼresizeʼ, handler) in the first and
...
(puthash "scroll" (quote (("name" . "scroll") ("signatures" "scroll" (("handler" "A function to execute each time the event is triggered.
...
(puthash "select" (quote (("name" . "select") ("signatures" "select" (("handler" "A function to execute each time the event is triggered.
...
collection of forms and/or form controls. The controls can be of
several types:
") (text . "") (html . "
<form>
...
form controls, such as <input>, <textarea>, and <select>. However, it
is typically easier to select the <form> element itself for
serialization:
") (text . "") (js . "
...
level of the DOM tree. However, if the original collection contains
more than one element, they might be mutual siblings and will both be
found. If you need an exclusive list of siblings, use
...
will trigger jQueryʼs internal effects tracking. In previous versions,
...
(puthash "submit" (quote (("name" . "submit") ("signatures" "submit" (("handler" "A function to execute each time the event is triggered.
...
<span></span>
") (text . "")) ((text . "") (text . "If youʼd like to prevent forms from being submitted unless a flag
variable is set, try:
...
(puthash "text" (quote (("name" . "text") ("signatures" "text" (("text" "The text to set as the content of each matched element. When Number or
Boolean is supplied, it will be converted to a String representation.

" nil nil)) (("function" "A function returning the text content to set. Receives the index
...
(puthash "toArray" (quote (("name" . "toArray") ("signatures" "toArray" nil) ("desc" (text . "Retrieve all the elements contained in the jQuery set, as an array.
...
(puthash "toggle" (quote (("name" . "toggle") ("signatures" "toggle" (("handler" "A function to execute every even time the element is clicked.

" nil nil) ("handler" "A function to execute every odd time the element is clicked.

" nil nil) ("handler" "Additional handlers to cycle through after clicks.
...
" "true" nil)) (("display" "Use true to show the element or false to hide it.
...
" nil nil) ("state" "A Boolean (not just truthy/falsy) value to determine whether the class
should be added or removed.

" nil nil)) (("state" "A boolean value to determine whether the class should be added or
removed.

" "true" nil)) (("function" "A function that returns class names to be toggled in the class
attribute of each element in the matched set. Receives the index
position of the element in the set, the old class value, and the state
as arguments.
" nil nil) ("state" "A boolean value to determine whether the class should be added or
removed.
...
") (text . "") (text . "Note: As with .triggerHandler() , when calling .trigger() with an event
name matches the name of a property on the object, prefixed by on (e.g.
triggering click on window that has a non null onclick method), jQuery
will attempt to invoke that property as a method.
") (text . "") (text . "Note: When triggering with a plain object that is not array-like but
still contains a length property, you should pass the object in an
array (e.g. [ { length: 1 } ]).
...
")) ("longdesc" (text . "") (text . ".triggerHandler( eventType ) executes all handlers bound with jQuery
for the event type. It will also execute any method called
on{eventType}() found on the element. The behavior of this method is
similar to .trigger() , with the following exceptions:
") (text . "") (text . "  * The .triggerHandler( \"event\" ) method will not call .event() on the
    element it is triggered on. This means .triggerHandler( \"submit\" )
    on a form will not call .submit() on the form.
...
" nil nil) ("handler" "The function that is to be no longer executed.
...
(puthash "unload" (quote (("name" . "unload") ("signatures" "unload" (("handler" "A function to execute when the event is triggered.
...
$( window ).unload(function() {
  return \"Handler for .unload() called.\";
    ...
    ") (text . "") (text . "This event is available so that scripts can perform cleanup when the
user leaves the page. Most browsers will ignore calls to alert(),
confirm() and prompt() inside the event handler. The string you return
may be used in a confirmation dialog, but not all browsers support
this. It is not possible to cancel the unload event with
.preventDefault().
...
")) ("longdesc" (text . "") (text . "The .unwrap() method removes the elementʼs parent. This is effectively
...
(puthash "val" (quote (("name" . "val") ("signatures" "val" (("value" "A string of text, a number, or an array of strings corresponding to the
...
") (text . "") (text . "val() allows you to pass an array of element values. This is useful
when working on a jQuery object containing elements like <input
type=\"checkbox\">, <input type=\"radio\">, and <option>s inside of a
<select>. In this case, the inputs and the options having a value that
matches one of the elements of the array will be checked or selected
while those having a value that donʼt match one of the elements of the
array will be unchecked or unselected, depending on the type. In case
of <input type=\"radio\">s that are part of a radio group and <select>s,
any previously selected element will be deselected.
...
") (text . "") (text . " [0042_04_04.png] figure 1
...
" nil nil)) (("function" "A function returning the width to set. Receives the index position of
...
structure to wrap around the matched elements. When you pass a jQuery
collection containing more than one element, or a selector matching
more than one element, the first element will be used.
" nil nil)) (("function" "A callback function returning the HTML content or jQuery object to wrap
...
elements.
" nil nil)) (("function" "A callback function which generates a structure to wrap around the
